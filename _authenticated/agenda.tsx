...
import { 
  CalendarDays, Plus, Loader2, Clock, CheckCircle2, XCircle, AlertTriangle, HelpCircle, ChevronLeft, ChevronRight, User, List, Calendar, Sliders, Trash2
} from "lucide-react";
...
function AgendaPage() {
  ...
  const createAppointmentMutation = useMutation({
    mutationFn: async () => {
      ...
      if (isBlock) {
        // 1. Find or create block client
        const { data: blockClients, error: blockClientErr } = await supabase
          .from("clients")
          .select("id")
          .eq("name", "Horário Bloqueado")
          .limit(1);
        
        if (blockClientErr) throw blockClientErr;

        if (blockClients && blockClients.length > 0) {
          finalClientId = blockClients[0].id;
        } else {
          const { data: newBlockClient, error: clientErr } = await supabase
            .from("clients")
            .insert({
          ...
          finalClientId = newBlockClient.id;
        }

        // 2. Find or create block service
        const { data: blockServices, error: blockServiceErr } = await supabase
          .from("services")
          .select("id")
          .eq("name", "Bloqueio de Horário")
          .limit(1);

        if (blockServiceErr) throw blockServiceErr;

        if (blockServices && blockServices.length > 0) {
          finalServiceId = blockServices[0].id;
        } else {
          const { data: newBlockService, error: serviceErr } = await supabase
            .from("services")
            .insert({
          ...
          finalServiceId = newBlockService.id;
        }
      ...
      const { error } = await supabase.from("appointments").insert({
        ...
        status: isBlock ? "confirmado" : "agendado",
        price: finalPrice,
        duration,
        notes,
      });
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["appointments"] });
      toast.success(isBlock ? "Horário bloqueado com sucesso!" : "Agendamento criado com sucesso!");
  ...
  const deleteAppointmentMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["appointments"] });
      toast.success("Horário desobstruído/excluído com sucesso!");
            ...
            <tbody className="divide-y text-sm">
              {appointments.length === 0 ? (
                ...
                appointments.map((appt) => {
                  const isBlockItem = appt.client?.name === "Horário Bloqueado";
                  return (
                    <tr key={appt.id} className={`hover:bg-muted/10 ${isBlockItem ? "bg-gray-100/50 dark:bg-gray-800/10 text-muted-foreground" : ""}`}>
                      <td className="p-3 font-medium">
                        {new Date(appt.start_time).toLocaleString("pt-BR", { dateStyle: "short", timeStyle: "short" })}
                      </td>
                      <td className="p-3 font-medium">{isBlockItem ? "🚫 Horário Fechado" : appt.client?.name}</td>
                      <td className="p-3">
                        <Badge variant="outline" style={{ borderColor: isBlockItem ? "#6b7280" : appt.service?.color, backgroundColor: (isBlockItem ? "#6b7280" : appt.service?.color) + "15" }}>
                          {isBlockItem ? "Indisponível" : appt.service?.name}
                        </Badge>
                      </td>
                      <td className="p-3">{appt.professional?.name}</td>
                      <td className="p-3 font-semibold">{isBlockItem ? "-" : `R$ ${Number(appt.price).toFixed(2)}`}</td>
                      <td className="p-3">
                        {isBlockItem ? (
                      ...
                      profAppts.map((appt) => {
                        ...
                        return (
                          <div 
                            key={appt.id} 
                            onClick={() => { setSelectedAppointment(appt); setIsDetailsDialogOpen(true); }}
                            className={`group relative cursor-pointer border rounded-xl p-3 hover:bg-muted/20 hover:border-tiffany/60 transition-all text-xs ${isBlockItem ? "bg-red-50/30 dark:bg-red-950/5 border-red-200/55 text-muted-foreground" : "bg-muted/10 hover:bg-muted/20"}`}
                          >
                            <div className="flex justify-between items-start">
                              ...
                              {isBlockItem ? (
                                <Badge variant="outline" className="px-1.5 py-0 text-[10px] bg-red-500/10 text-red-600 border-red-200">
                                  Bloqueado
                                </Badge>
                              ) : (
                            ...
                            <h4 className="font-serif text-sm font-semibold text-foreground mt-1 flex items-center gap-1">
                              {isBlockItem ? "🚫 Horário Fechado" : appt.client?.name}
                            </h4>
                            <p className="text-muted-foreground mt-0.5">{isBlockItem ? "Indisponível para atendimento" : appt.service?.name}</p>
                            {appt.notes && (
        ...
        <DialogContent className="sm:max-w-[460px]">
          ...
          <form onSubmit={(e) => { e.preventDefault(); createAppointmentMutation.mutate(); }} className="space-y-4 py-2">
            <div className="flex items-center space-x-2 border p-3 rounded-lg bg-muted/20">
              ...
              />
              <Label htmlFor="is-block" className="cursor-pointer font-medium text-xs">
                Bloquear este horário (Indisponibilidade / Folga)
              </Label>
            </div>
            ...
            <div className="space-y-1">
              ...
              <Textarea
                id="notes"
                placeholder={isBlock ? "Ex: Almoço, Dentista, Viagem..." : "Ex: alergias, mapping desejado, etc."}
                rows={2}
                value={notes}
        ...
        <DialogContent className="sm:max-w-[460px]">
          ...
          {selectedAppointment && (
            <div className="space-y-4 py-2 text-sm text-foreground">
              {selectedAppointment.client?.name === "Horário Bloqueado" ? (
                <div className="space-y-4">
                  <div className="bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-900 p-4 rounded-xl flex items-center gap-3">
                    <AlertTriangle className="h-6 w-6 text-red-600 flex-shrink-0" />
                    <div>
                      <h4 className="font-semibold text-red-800 dark:text-red-300">Horário Bloqueado</h4>
                      <p className="text-xs text-red-700 dark:text-red-400">Esta profissional está indisponível neste período.</p>
                    </div>
                  </div>
                  ...
                  {selectedAppointment.notes && (
                    <div className="bg-muted/30 p-2.5 rounded-lg border">
                      <span className="text-xs font-semibold text-muted-foreground block">Motivo / Notas:</span>
                      <p className="text-xs whitespace-pre-wrap">{selectedAppointment.notes}</p>
                    </div>
                  )}

                  <div className="flex gap-2 justify-end border-t pt-3.5">
                    <Button type="button" variant="outline" size="sm" onClick={() => setIsDetailsDialogOpen(false)}>Fechar</Button>
                    <Button 
                      type="button" 
                      onClick={() => {
                        if (confirm("Deseja realmente desobstruir este horário?")) {
                          deleteAppointmentMutation.mutate(selectedAppointment.id);
                        }
                      }}
                      className="bg-destructive hover:bg-destructive/90 text-white rounded-full flex items-center gap-1.5"
                      size="sm"
                      disabled={deleteAppointmentMutation.isPending}
                    >
                      {deleteAppointmentMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <><Trash2 className="h-4 w-4" /> Desbloquear Horário</>}
                    </Button>
                  </div>
                </div>
              ) : (
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="flex items-end justify-end">
                      <Button 
                        ...
                        className="text-destructive hover:bg-destructive/10 hover:text-destructive h-8 px-3 rounded-full text-xs"
                      >
                        <Trash2 className="mr-1 h-3.5 w-3.5" /> Excluir permanentemente
                      </Button>
                    </div>
                  </div>
